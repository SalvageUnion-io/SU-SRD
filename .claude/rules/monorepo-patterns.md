...
- `apps/srd/` - Static SRD reference site. **Not Astro** — an in-house SSG at `apps/srd/ssg` (`build.ts` / `dev.ts` / `render.tsx`) over Vite, with React 19 islands and Tailwind v4. Routes are `src/pages/**/*.page.tsx` modules registered in `ssg/routes.ts`; islands mount with `createRoot`. There is **no whole-page-output gate** — the Astro-migration parity script is retired, so verify emit changes against real built output. Contract: [`apps/srd/ssg/DESIGN.md`](../../apps/srd/ssg/DESIGN.md)
