...
- `apps/srd/` - Static SRD reference site. **Not Astro** — an in-house SSG at `apps/srd/ssg` (`build.ts` / `dev.ts` / `render.tsx` / `parity.ts`) over Vite, with React 19 islands and Tailwind v4. Routes are `src/pages/**/*.page.tsx` modules registered in `ssg/routes.ts`; islands mount with `createRoot`; `ssg/parity.ts` is the acceptance gate. Contract: [`apps/srd/ssg/DESIGN.md`](../../apps/srd/ssg/DESIGN.md)
