...
2. The data file (`data/<id>.json`). The `schemas/index.json` catalog entry is **generated** — `itemCount`, `requiredFields`, `title` and `displayName` are derived from the data file and the JSON Schema by `tools/generateDocs.ts`, which `build:package` now runs. Write the schema's prose in the Zod schema's `.describe()`; it flows Zod → `schemas/<id>.schema.json` → `schemas/index.json`. Only the `meta` flag is preserved from the existing entry.
...
Then run `bun run build:package` (or `bun --filter salvageunion-reference scaffold:entity <schema-id> [Singular] [Plural]` first, to print an exact checklist with ready-to-paste snippets for steps 1–3 — that one is a package-level script, not a root one). `tools/generateRegistry.ts` derives everything else — `dataLoaders`/`zodSchemaMap`/`schemaDisplayNames` in `lib/generated/modelFactoryRegistry.generated.ts`, and the `LazyModel` instances/`lazyModelMap`/`SchemaToEntityMap`/`SCHEMA_REGISTRY`/`SalvageUnionReference` static accessor in `lib/generated/schemaRegistry.generated.ts` plus the marker-injected block in `lib/index.ts` — from that one manifest entry. Run `bun test` afterward; `lib/registryConsistency.test.ts` independently re-verifies every generated registry covers the same schema-id set.
