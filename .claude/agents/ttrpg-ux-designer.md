...
- **React 19** with **TanStack Router**, bundled by **Vite** (`apps/itun`); `apps/srd` is statically generated by an **in-house SSG** at `apps/srd/ssg` (**not Astro** — that was migrated away) with React 19 islands mounted via `createRoot`
