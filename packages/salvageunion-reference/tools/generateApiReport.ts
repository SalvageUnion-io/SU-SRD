 ...
 * whole-tree dump. `tsconfig.api.json` carries the same roots in its
 ...
 *
 * Two things are deliberately left OUT, because they made the report a 770 KB
 * file nobody could review (audit PK-09):
 *
 *   - The Zod schema declarations under `lib/schemas/` (everything except the
 *     `lib/schemas/index.d.ts` barrel). They were ~70% of the report, they are
 *     not on the main barrel, and their SHAPE is already gated — more readably
 *     — by the committed `schemas/*.schema.json` files that `check generated`
 *     diffs.
 *   - Comments (`removeComments` in `tsconfig.api.json`). JSDoc prose is not API,
 *     and a reworded comment should not read as an API change.
 *
 * What remains is every public barrel, as an explicit name list, plus the
 * declarations of the modules those names come from.
 */
...
/** A Zod schema declaration file — see the header for why these are excluded. */
function isZodSchemaDeclaration(rel: string): boolean {
  return rel.startsWith('lib/schemas/') && rel !== 'lib/schemas/index.d.ts'
}

const files = collectDeclarations(apiDtsLibDir).filter(
  (abs) => !isZodSchemaDeclaration(relative(apiDtsDir, abs).split(sep).join('/'))
)
if (files.length === 0) {
