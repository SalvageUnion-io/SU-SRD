 ...
 */
export function ReferenceEntityListDisplay({
  choice,
  selectedChoice,
  isMultiSelect = false,
  spacing,
}: ReferenceEntityListDisplayProps) {
  ...
  return (
    <div style={{ marginTop: `${spacing.contentPadding + 1}rem` }} className="space-y-4">
      {visibleEntities.map((entity, idx) => {
        ...
        // ReferenceEntityDisplay only accepts SURefEntity, so we need to check if entity is a valid entity
