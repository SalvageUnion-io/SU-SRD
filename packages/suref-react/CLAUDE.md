...
- **The four groups have crisp definitions** (sidebar order, top-to-bottom):
  ...
  - **Atoms** — indivisible primitives: one job, compose no other atom (Text, Slab, Badge — the one label-chip, `shape="chip"` or `"stamp"`; Button, Tooltip, Glyph, Vital Gauge, Stat, Bay Status, Condition Swatch, Inline Ref/Edit Field, Slot Grid, Activation Cost, Sheet, Field, Panel).
  ...
  - **Compositions** — domain/game components built from atoms (the Reference Entity family, Choice Groups, Class Ability Tree, Roll Table, Entity Grid/Row, Content).
