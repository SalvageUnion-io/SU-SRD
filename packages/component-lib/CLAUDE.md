...
- **The taxonomy is enforced too.** A second assertion in `src/story-coverage.test.ts` requires every story's **meta** `title:` to start with a sanctioned top-level group (`^(Foundations|Atoms|Containers|Compositions)/…`) — no un-prefixed junk-drawer titles. It reads only the default-export meta title, so `title:` strings inside story bodies are ignored. Introduce a new top-level group only by extending both that pattern and the `storyOrder` list in `.ladle/config.mjs`.
- **The four groups have crisp definitions** (sidebar order, top-to-bottom):
  ...
  - **Atoms** — indivisible primitives: one job, compose no other atom (Text, Slab, Badge — the one label-chip, `shape="chip"` or `"stamp"`; Button, Tooltip, Glyph, Vital Gauge, Stat, Bay Status, Condition Swatch, Inline Ref/Edit Field, Slot Grid, Activation Cost, Sheet, Field, Panel, Search Field, Catalog Tile, Kofi Button).
  ...
  - **Compositions** — domain/game components built from atoms (the Reference Entity family, Choice Groups, Class Ability Tree, Roll Table, Entity Grid/Row, Content, App Bar, Footer, Static Entity Content, Nav Drawer, Wiz Shell).
