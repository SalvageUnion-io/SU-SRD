...
| `check-doc-drift.ts` | `validate:doc-drift` | Live docs, skills, rules, agent memory and workflow prompts stay true: exports map, workspace list, superseded ADRs, component-lib symbols, framework majors, `bun run` scripts, counts, MCP servers, ADR routing, and every cited repo path existing. | Fix the doc. If a citation is deliberately historical, say so in the same sentence ("was deleted"). | — |
