...
| `workflows` | `check-workflows.ts` | One `Bun.YAML` parse of `.github/`, five checks: `aggregator` (`CI Success` needs every ci.yml job), `path-filters` (every app's `workspace:*` dependency is in its filter group), `pinning` (third-party actions SHA-pinned; undeclared `bunx` tools version-pinned), `bun-version` (`.bun-version` = root `bun-types` = the running Bun; no inline pins), `convex-guard` (the deploy still runs `convex deploy` and refuses a missing `CONVEX_DEPLOY_KEY`). `--only=` runs a subset. | The message names the file and the fix. | — |
