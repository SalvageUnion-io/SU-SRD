...
- **Cloudflare** — hosts **everything**: `apps/srd` (`salvageunion.io`), `apps/itun` (`intheunionnow.com`), `apps/su-assets` (`assets.salvageunion.io`) and the Discord bot, all on Workers, with two R2 buckets (`su-itun-snapshots`, `su-lp-assets`) and Images for artwork derivatives. Config is `apps/*/wrangler.jsonc`; deploys run from `.github/workflows/deploy-cloudflare.yml`. No Cloudflare MCP server is declared yet — P8 adds `bindings` and `observability`; until then use `wrangler`.
- **Netlify** — **hosts nothing.** Every `netlify.toml`, both `netlify/` function trees and `@netlify/blobs` are deleted. The account still exists and its three sites still answer on their `.netlify.app` hostnames, serving a stale build; that is decommission debris, not an origin. One tool still reaches it — `tools/sync-snapshots-to-r2.ts`, kept because P6's delta run was never performed and the Blobs store is its source. MCP server: official `@netlify/mcp` over stdio via `npx`; retire it with the account.
...
- **Render** — **no longer serves production.** The bot moved to HTTP interactions on a Cloudflare Worker (ADR-033 P5, live 2026-08-19); `render.yaml` is deleted; the service itself remains until P8 retires the account. MCP server: hosted HTTP at `https://mcp.render.com/mcp`.
