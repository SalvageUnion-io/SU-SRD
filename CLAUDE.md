...
- [`docs/adrs/`](docs/adrs/) — architecture decision records, **35 of them** (ADR-001 through ADR-035). Consult the matching ADR before revisiting a prior decision, and **read its `## Status` header first** — several are superseded and the supersession is only recorded there (ADR-001 → ADR-030; ADR-012 → ADR-031; ADR-023 → ADR-027 → ADR-028; ADR-030 §1 → ADR-034; ADR-034's terminal-decline consequence → ADR-035). The three that govern:
...
- **Sentry** — error tracking across every surface: `@sentry/browser` in `srd` and `itun`, `@sentry/cloudflare` in all three Workers via `observability/cloudflare`, and `@sentry/vite-plugin` for `itun` and `srd` sourcemaps (both gated on `SENTRY_AUTH_TOKEN`, so only the deploy build uploads). `@sentry/node` is **gone** — its only consumer was the bot's dormant Node gateway, deleted once Render was retired. MCP server: remote HTTP at `https://mcp.sentry.dev/mcp`.
...
- `packages/observability/` - Sentry wiring: `/cloudflare` for the three Workers (`withObservability`, `reportError`, the cron check-in; tested against the real SDK) and `/browser` for `createBrowserObservability`, the init/capture half the two browser shims share (imports no Sentry code — each app keeps only its DSN guard, which is what tree-shakes the SDK). Also `/worker-http`: the `edgeCache()` and base security headers itun's and su-assets' Workers share. The `/node` subpath is **deleted**: the Discord bot's Node gateway was its only consumer. Each Worker's hand-written `Env` is held to its `wrangler.jsonc` bindings by `bun run check:worker-env` (in `validate:all`), which reads them from `wrangler types`.
