...
Discord bot for rolling on Salvage Union tables, built with:
- **discord.js v14** - Discord API library
- **salvageunion-reference** - Roll table data access
- **TypeScript** - Type-safe development

#### Key Features

- `/roll [table]` - Roll on any Salvage Union table with autocomplete
- Deployed via Render as a worker service
...
# Deploy slash commands to test guild
...
- **Build Command**: `bun install && bun run build:package && bun --filter @suref/discord-bot build`
- **Start Command**: `node apps/discord-bot/dist/index.js`
- **Environment Variables**: `DISCORD_TOKEN`, `DISCORD_CLIENT_ID`, `DISCORD_GUILD_ID` (optional)
