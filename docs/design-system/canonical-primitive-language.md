...
- **Stat fusion (complete).** The stat cluster is now ONE component.
  ...
  - `orientation="horizontal"` + `pips` → the condensed inline `[label · pips · value]` chip (was **MiniStat**)
  - `pips` (or `states[]`) → the framed tracker: code tab, numeral + steppers, pip track / bay tally, unit bar, tones + heat (was **StatBlock**)
...
| **Card**            | `components/shared/Card.tsx`          | the card shell + tabs                                                                                                 | `tone` · `mode` (full / compact / head / tooltip / instrument) · `tabs` · `footMeta` · `footActions` · `spine`                                                                                                                                       |
...
3. **Ladle catalog** — one story per primitive (a browsable catalog). Today only
   ...
   the codex), `Badge`, `VitalGauge`, `Card`, `RollTable`, `Modal`, …
