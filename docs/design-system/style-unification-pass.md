...
One canonical primitive language. Every surface (`suref-web`, `in-the-union-now`,
`suref-react`) conforms to the same vocabulary, tokens, and prop APIs. The words
...
2. **Lift shared vocabulary up; apps only re-assemble.** Presentational
   primitives live in `packages/suref-react` (which owns the **one** Ladle
   catalog). `suref-web` and `in-the-union-now` are **consumers** — after a lift
...
The Ladle catalog is **suref-react-only** and reorganizing into single-page
...
1. **Lift** the shared presentation from ITUN into `packages/suref-react` (keep
...
3. **Migrate** the ITUN (and suref-web) call sites onto the shared primitive.
...
- The Ladle `bun patch` and suref-web's pinned TS6 are load-bearing — don't
