# ADR-011: `suref-react` Ships as TypeScript Source (No Build Step)
...
`suref-react` is the shared component library consumed by both `suref-web`
...
`suref-react` has **no build step**. It exports TypeScript source directly:
...
- Consumers must include `suref-react`'s source in their compile/Tailwind
