# ADR-012: `suref-web` as an Astro Static Site with React Islands
...
`suref-web` is built with **Astro 5**, pre-rendering all entity reference pages
...
- Shared UI comes from `suref-react` ([ADR-011](ADR-011-suref-react-source-no-build.md));
