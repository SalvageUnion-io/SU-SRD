...
/**
 * The `/*` block of `apps/itun/public/_headers`, in code.
 *
 * WHY IT HAS TO EXIST TWICE. Cloudflare's Static Assets docs are explicit that
 ...
 * comment records that this exact class of gap already cost production once.
 *
...
const SECURITY_HEADERS: Record<string, string> = {
  ...
  'content-security-policy':
    ...
    "connect-src 'self' https://*.ingest.de.sentry.io https://*.convex.cloud " +
    'wss://*.convex.cloud https://*.convex.site;',
}
 ...
 * only way. `response.body` is null for 204/304, which `new Response` accepts.
 */
