 ...
 *
 * `ActiveItemBand` is the rules + store half, per band (mech / pilot /
 * crawler). It dispatches by mount state (playStateStore), runs the reactor /
 * damage / egress verbs through the pure rules engine (`dashboardRules.ts` →
 * `lib/rules/*`) and the entity store under the ADR-007 automation boundary,
 * then hands a pure `ActiveItemBandModel` to `ActiveItemBandFrame`, which only
 * renders it. The frame lived in component-lib with ITUN as its only consumer,
 * imported `as ActiveItemBandView`; one file since the component-lib boundary
 * audit (PK-03). `StorageBay` composes overlay bodies.
 *
...
import { Badge, Button, CountStepper, cn, RuleBrief } from 'component-lib'
...
/**
 * The presentational half: renders the bays + gauges + buttons + the resolve
 * overlay from a computed `ActiveItemBandModel`. Exported for the Ladle story,
 * which drives it with fixture view-models and no store.
 */
