...
export function useUpsertPlayerChoice() {
  ...
  return useMutation({
    ...
    onSuccess: (choice) => {
      ...
      // Find and invalidate only the specific parent's entity query
      const allEntityQueries = queryClient.getQueriesData<HydratedEntity[]>({
        queryKey: entitiesKeys.all,
      })
      for (const [queryKey, entities] of allEntityQueries) {
        if (!entities) continue
        const targetId = entityId || choiceId
        if (targetId && entities.some((e) => e.id === targetId)) {
...
export function useDeletePlayerChoice() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      const targetId = variables.entityId || variables.choiceId
      for (const [queryKey, entities] of allEntityQueries) {
        if (!entities) continue
        if (targetId && entities.some((e) => e.id === targetId)) {
          queryClient.invalidateQueries({ queryKey })
