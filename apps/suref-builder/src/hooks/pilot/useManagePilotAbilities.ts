...
type AlertFn = (options: { message: string }) => Promise<void>

export function useManagePilotAbilities(
  id: string | undefined,
  confirm: ConfirmFn,
  showAlert: AlertFn
) {
  ...
  const handleAddAbility = useCallback(
    async (abilityId: string) => {
      ...
      if ((pilot?.current_tp ?? 0) < cost) {
        await showAlert({ message: `Not enough TP. This ability costs ${cost} TP.` })
        return
      }
    ...
    [id, pilot, selectedClass, selectedAdvancedClass, createEntity, updatePilot, showAlert]
  )
  ...
  const handleRemoveAbility = useCallback(
    async (entityId: string) => {
      ...
      const confirmed = await confirm({
        message: `Are you sure you want to remove "${abilityName}"? It will cost 1 TP.`,
      })
      if (confirmed) {
        deleteEntity.mutate(
    ...
    [id, pilot, abilities, deleteEntity, updatePilot, confirm]
  )
