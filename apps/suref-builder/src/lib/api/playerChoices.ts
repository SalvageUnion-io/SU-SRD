 ...
 *
 * For single-select choices: uses Supabase upsert with onConflict to atomically
 * insert or update based on unique constraints (entity_id + choice_ref_id or
 * player_choice_id + choice_ref_id).
...
export async function upsertPlayerChoice(
  ...
  // Use the appropriate unique constraint for atomic upsert
  const onConflict = data.entity_id ? 'entity_id,choice_ref_id' : 'player_choice_id,choice_ref_id'

  const { data: choice, error } = await supabase
    .from('player_choices')
    .upsert(validated, { onConflict })
    .select()
    .single()
