...
function EquipmentItemWithSlots({
  ...
  const removeButtonConfig = {
    ...
    onClick: async () => {
      const confirmed = await confirm({
        message: `Are you sure you want to remove "${item.equipment.name}"?`,
      })
      if (confirmed) {
  ...
  const systemRemoveButtonConfig = (system: SURefSystem, systemEntity: HydratedEntity) => ({
    ...
    onClick: async (e: React.MouseEvent) => {
      e.stopPropagation()
      const confirmed = await confirm({
        message: `Are you sure you want to remove "${system.name}"?`,
      })
      if (confirmed) {
  ...
  const moduleRemoveButtonConfig = (module: SURefModule, moduleEntity: HydratedEntity) => ({
    ...
    onClick: async (e: React.MouseEvent) => {
      e.stopPropagation()
      const confirmed = await confirm({
        message: `Are you sure you want to remove "${module.name}"?`,
      })
      if (confirmed) {
...
export function PilotInventory({ id, disabled = false, readOnly = false }: PilotInventoryProps) {
  ...
  const { handleAddEquipment, handleRemoveEquipment } = useManagePilotInventory(id, confirm)
