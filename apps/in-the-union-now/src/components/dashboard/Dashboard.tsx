...
/** Dashboard row meta sub-line for a mech: "Chassis · TL n". */
function mechMeta(chassisRef: string): string | undefined {
  try {
...
/** Dashboard row meta sub-line for a crawler: "TL n · m bays". */
function crawlerMeta(techLevel: string, bayCount: number): string {
    ...
    <main className="mx-auto max-w-7xl p-4 sm:p-6">
      <div className="mb-8 border-b-2 border-[var(--color-su-black)] pb-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <h1 className="font-cond text-3xl font-bold uppercase tracking-[.04em]">
            ITUN — Saved Builds
          </h1>
          <WorkspaceSwitcher
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          ...
          <section aria-labelledby="pilots-heading">
            <div className="mb-3 flex items-center justify-between gap-2">
              <h2
                id="pilots-heading"
                className="font-cond text-xl font-bold uppercase tracking-[.1em] text-[var(--color-su-orange-dark)]"
              >
                Pilots
              </h2>
              <a
              ...
              <ul className="flex flex-col gap-2">
                {pilots.map((p) => (
                  <EntityListItem
                    ...
                    entityType="pilot"
                    meta={[p.callsign ? `"${p.callsign}"` : null, resolveClassName(p.classRef)]
                      .filter(Boolean)
                      .join(' · ')}
                  />
                ))}
              ...
              <ul className="flex flex-col gap-2">
                {crawlers.map((c) => (
                  <EntityListItem
                    ...
                    entityType="crawler"
                    meta={crawlerMeta(c.techLevel, c.crawlerBays?.length ?? 0)}
                  />
                ))}
              </ul>
            )}
...
function EmptyState({ label, ctaHref, ctaLabel }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center gap-3 rounded-[3px] border-[1.5px] border-dashed border-[#a7b4bd] px-4 py-6 text-center text-[var(--color-su-grey-dark)]">
