      ...
      <div className="flex items-center justify-between text-xs">
        ...
        <span
          className={cn(
            ...
            isOver ? 'font-semibold text-roll-cascade' : 'text-su-black'
          )}
        >
          {used}&nbsp;/&nbsp;{max}
        </span>
      </div>
      <div className="h-1.5 rounded-full bg-su-blue-pale">
        <div
          className={cn(
            'h-full rounded-full transition-all',
            isOver ? 'bg-roll-cascade' : 'bg-su-orange-dark'
          )}
          style={{ width: `${pct}%` }}
        />
      </div>
...
function ViolationBadge({ 'data-testid': testId, message }: ViolationBadgeProps) {
  return (
    <div
      data-testid={testId}
      className="rounded-[3px] border border-su-rust bg-su-rust/10 px-2 py-1 text-xs text-su-rust"
    >
      {message}
    </div>
  )
...
export function CapacityIndicator({
  ...
  return (
    <div
      className={cn(
        'rounded-md border border-su-black bg-white p-4 flex flex-col gap-3',
        className
      )}
