...
export function AppHeader({ onSearchClick }: AppHeaderProps) {
  return (
    <header className="flex items-center gap-3 border-b-[3px] border-su-orange-dark bg-su-ink-dark px-4 py-2.5 sm:px-8 sm:py-3">
      ...
      <AppLink href="/" className="flex min-w-0 shrink-0 items-center gap-3 no-underline">
        ...
        <span className="flex min-w-0 flex-col">
          <span className="font-cond text-xl font-bold leading-none tracking-[0.005em] text-su-paper sm:text-2xl">
            ITUN
            <span className="ml-1.5 inline-block rounded bg-rust px-1.5 py-0.5 align-[0.3em] font-cond text-[10px] font-bold uppercase leading-none tracking-[0.12em] text-su-paper">
              Beta
            </span>
          </span>
      ...
      {/* Right side: search trigger + outbound link to the SRD reference site */}
      <div className="ml-auto flex shrink-0 items-center gap-3 sm:gap-4">
        {onSearchClick && (
          <button
            ...
            className="flex cursor-pointer items-center gap-1.5 rounded-[3px] border border-su-paper/40 px-2.5 py-1.5 font-cond text-sm font-semibold uppercase tracking-[0.06em] text-su-paper transition-colors hover:border-su-orange hover:text-su-orange"
          >
            <Search className="size-3.5" aria-hidden="true" />
            <span className="hidden sm:inline">Search</span>
            <kbd
              aria-hidden="true"
              className="hidden rounded-[3px] border border-su-paper/40 px-1 py-0.5 font-mono text-[10px] leading-none sm:inline"
            >
              {IS_APPLE ? '⌘K' : 'Ctrl K'}
            </kbd>
          </button>
        ...
        <a
          ...
          className="shrink-0 font-cond text-sm font-semibold uppercase tracking-[0.06em] text-su-paper no-underline transition-colors hover:text-su-orange"
        >
